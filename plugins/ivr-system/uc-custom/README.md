...
The setup_wazo_plugin function in the boilerplate code outlines the conceptual steps for deployment:

1. **Create Configuration Directory:** Ensures /etc/wazo-ivr-plugin (or the path specified by WAZO_IVR_CONFIG_DIR) exists.  
...
6. **Initial IVR Flow:** A default ivr_flows.json file is created if one doesn't exist, providing a basic IVR structure.

### **Configuration**

The IVR flow is defined in a JSON file, by default located at /etc/wazo-ivr-plugin/ivr_flows.json. An example structure is provided in the setup_wazo_plugin function. You can modify this file to define your desired IVR menus, options, and actions.

**Example ivr_flows.json structure:**
...
{  
    ...
    "voicemail_menu": {  
        ...
        "action": "transfer_to_extension",  
        "target": "voicemail_ext"  
    }  
}

## **Wazo Dialplan Integration**

To make the IVR plugin functional, you must configure a custom dialplan context in Asterisk that routes incoming calls to your plugin's main script. This is the final step in the manual deployment process.

1. **Define the Dialplan Context:** A new context, such as [app-wazo-ivr], is created in the file /etc/asterisk/extensions.d/02-wazo-ivr-plugin.conf. This context uses the Asterisk AGI (Asterisk Gateway Interface) command to execute your plugin's main Python script.  
2. **Route Incoming Calls:** In the Wazo web interface, you must configure an **Incall** to point to this app-wazo-ivr context. This ensures that any calls matching that incall route are handed off to your plugin for processing.

The dialplan entry would look like this:

[app-wazo-ivr]  
exten => s,1,AGI(/var/lib/wazo-provd/plugins/wazo-ivr-plugin/main.py)

## **Usage**

Once deployed and configured, the plugin will intercept incoming calls configured to use its IVR services. The handle_incoming_call method is the entry point for processing a call through the defined IVR flow.

## **Development Notes**

* **Recent Modifications:** This README has been updated to reflect recent development fixes, including the addition of from typing import Dict, Any and import logging to correctly define and use these modules.  
...
You can test using the demo SIP server.

Please login with these SIP credentials. You can use any known SIP phone like zoiper.

```
SIP domain: wazo-adam-demo.lbackups.com
user: 1000
secret: 8STkEuxCLVe7Ckfp
```

You can test the IVR by dialing 5000
